...
namespace FrameWork.Asset
{
    public static class FilePathTools
    {
        ...
        // 根据全路径的正则表达式筛选
        public static string[] SelectEntriesEx(string[] entries, string selectFullPattern)
        {
            List<string> selectedEntries = new List<string>();
            foreach (string entry in entries)
            {
                string path = NormalizePath(entry);
                if (Regex.IsMatch(path, selectFullPattern, RegexOptions.IgnoreCase))
                {
                    selectedEntries.Add(path);
                }
            }
        ...
        // 根据正则表达式获取目录下的文件夹
        public static string[] GetDirectoriesEx(string path, string regexPatternPath, SearchOption searchOption = SearchOption.TopDirectoryOnly)
        {
            try
            {
                string[] directories = Directory.GetDirectories(path, "*", searchOption);
                return SelectEntriesEx(directories, regexPatternPath);
            }
            catch (Exception e)
            {
                DebugUtil.LogWarning("Get directories failed from : " + path + "\n" + e);
        ...
        public static bool DeleteDirectory(string path, bool recursive = false)
        {
            ...
            {
                Debug.LogWarning("Delete directory failed : " + path + "\n" + e);
                return false;
            }
        }
